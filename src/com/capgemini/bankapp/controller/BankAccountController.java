...
public class BankAccountController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (path.contentEquals("/getAllBankAccounts.do")) {

			List<BankAccount> bankAccounts = service.findAllBankAccounts();
			RequestDispatcher dispatcher = request.getRequestDispatcher("getAllBankAccounts.jsp");
			request.setAttribute("accounts", bankAccounts); // Passing the bank account object
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		else if (path.contentEquals("/searchAllBankAccounts.do")) {
			long accountId = Long.parseLong(request.getParameter("account_id"));
			BankAccount account;
			try {
				account = service.searchBankAccount(accountId);
				System.out.println(account);
				RequestDispatcher dispatcher = request.getRequestDispatcher("search.jsp");
				request.setAttribute("accounts", account);
				dispatcher.forward(request, response);
			} catch (BankAccountNotFoundException e) {
				out.print("<h2> Account not found <h2>");
				e.printStackTrace();
			}

		}

		else if (path.contentEquals("/updateBankAccounts.do")) {
			long accountId = Long.parseLong(request.getParameter("account_id"));
			BankAccount account;
			try {
				account = service.searchBankAccount(accountId);
			 ...
			 else if (path.contentEquals("/updateAccount.do")) {
			  int accountId = Integer.parseInt(request.getParameter("accountid"));
			  String updatedName= request.getParameter("updatedname");
			  String updatedType = request.getParameter("updatedtype");
			 
			  if (service.updateBankAccountDetails(accountId, updatedName, updatedType))
			  {
			   response.sendRedirect("getAllBankAccounts.do");
			  }
			  else
			  {
			   out.print("<h2> Failed to update <h2>");
			   out.close();
			  }
			 }
